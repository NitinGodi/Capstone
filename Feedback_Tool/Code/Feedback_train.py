...
# Training and testing LSTM model
    ...
    for epoch in range(CONF['lstm_epochs']):
        ...
        if new_f1 > best_f1:
            ...
            early_stop = 0
            torch.save(lstm_model.state_dict(), f'best_lstm_model.bin')
        else:
            early_stop += 1
...
# Training and testing Roberta model
    ...
    for epoch in range(CONF['CONFIG']['epochs']):
        ...
        util.validate(roberta_model, preprocessed_data['df'], preprocessed_data['df_val'], preprocessed_data['dl_val'], epoch, preprocessed_data['valid_idx'], preprocessed_data['unique_ids'])
